...
function MemoWidget({ initialContent, onCreated }: MemoWidgetProps): React.ReactElement {
  ...
  const nextMemo = useMemo((): MemoItem | null => {
    return memos.find((m) => m.remindAt > currentTime) || null
  }, [memos, currentTime])
  ...
  return (
    <div className="memo-widget">
      <div className="memo-widget__toggle" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="memo-widget__toggle-left">
          <svg
            className={`memo-widget__chevron ${isExpanded ? 'memo-widget__chevron--expanded' : ''}`}
            width="12"
            height="12"
            viewBox="0 0 24 24"
            fill="none"
          ...
          <span className="memo-widget__toggle-title">备忘录</span>
          {memos.length > 0 && <span className="memo-widget__badge">{memos.length}</span>}
        </div>
        {nextMemo && !isExpanded && (
          <div className="memo-widget__next">
            <span className="memo-widget__next-title">{nextMemo.title}</span>
            <span className="memo-widget__next-time">{getTimeUntil(nextMemo.remindAt)}</span>
          </div>
        )}
      </div>

      {isExpanded && (
        <div className="memo-widget__content">
          <button
            className="memo-widget__add-btn"
          ...
          {showCreateForm && (
            <div className="memo-widget__create-form">
              <input
                type="text"
                placeholder="标题"
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                className="memo-widget__input"
              />
              <textarea
                placeholder="内容（可选）"
                value={content}
              ...
              <button
                className={`memo-widget__time-btn ${
                  selectedMinutes !== null || selectedDate ? 'memo-widget__time-btn--set' : ''
                }`}
              ...
              <div className="memo-widget__form-actions">
                <button onClick={() => setShowCreateForm(false)} className="memo-widget__btn">
                  取消
                </button>
                <button
                  onClick={handleCreateMemo}
                  className="memo-widget__btn memo-widget__btn--primary"
          ...
          <div className="memo-widget__list">
            {memos.length === 0 ? (
              <div className="memo-widget__empty">
                <svg
                  width="32"
                  height="32"
                  viewBox="0 0 24 24"
                ...
                <span className="memo-widget__empty-text">暂无备忘录</span>
                <span className="memo-widget__empty-hint">点击上方按钮创建</span>
              </div>
            ) : (
              memos.map((memo) => (
                <div key={memo.id} className="memo-widget__item">
                  <div className="memo-widget__item-header">
                    <div className="memo-widget__item-left">
                      <button
                        className={`memo-widget__checkbox ${memo.isCompleted ? 'memo-widget__checkbox--checked' : ''}`}
                      ...
                      <span
                        className={`memo-widget__item-title ${memo.isCompleted ? 'memo-widget__item-title--completed' : ''}`}
                      >
                        {memo.title}
                      </span>
                    </div>
                    <button
                      className="memo-widget__item-delete"
                  ...
                  {memo.content && <div className="memo-widget__item-content">{memo.content}</div>}
                  <div className="memo-widget__item-time">
                    <svg
                      width="10"
                      height="10"
                      viewBox="0 0 24 24"
